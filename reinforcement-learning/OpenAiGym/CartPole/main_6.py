...
class Agent(object):
    ...
    def learn(self, batch=32, gamma=0.95):
        ...
        next_values = self.modelPre.predict(np.array(next_states))
...
if __name__ == '__main__':
    root = getDataFilePath(f'CartPole/CartPole_6/')
    if not os.path.exists(root):
        os.mkdir(root)
    startTime = time.time()
    totalEpisode = 0
    for i in range(5):
        cartPole = CartPole('CartPole-v0', os.path.join(root, f'CartPole_6_{i + 1}'))
