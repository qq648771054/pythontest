...
class MountainCar(MountainCarBase):
    ...
    def train(self, showProcess=False):
        agent = Agent(self, memorySize=10000)
        agent.model.summary()
        spendTime = 0
        episode = 0
        steps = [200] * 10
...
if __name__ == '__main__':
    root = getDataFilePath(f'MountainCar/MountainCar_3/')
    if not os.path.exists(root):
        os.mkdir(root)
    startTime = time.time()
    totalEpisode = 0
    for i in range(5):
        cartPole = MountainCar('MountainCar-v0', os.path.join(root, f'MountainCar_3_{i + 1}'))
