...
class MountainCar(MountainCarBase):
    ...
    def train(self, showProcess=False):
        ...
        while True:
            ...
            agent.learn()
            agent.syncModel()
            self.log(f'episode {episode}: step {step}, position {state[0]}')
...
if __name__ == '__main__':
    root = getDataFilePath(f'MountainCar/MountainCar_1/')
    if not os.path.exists(root):
        os.mkdir(root)
    startTime = time.time()
    totalEpisode = 0
    for i in range(5):
        cartPole = MountainCar('MountainCar-v0', os.path.join(root, f'MountainCar_1_{i + 1}'))
