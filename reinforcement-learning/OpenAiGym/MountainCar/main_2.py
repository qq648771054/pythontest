...
class Agent(object):
    ...
    def chooseAction(self, state):
        value = self.model.predict(np.array([state]))[0]
        return np.random.choice(self.env.actionLen, p=softmax(value))
...
if __name__ == '__main__':
    root = getDataFilePath(f'MountainCar/MountainCar_2/')
    if not os.path.exists(root):
        os.mkdir(root)
    startTime = time.time()
    totalEpisode = 0
    for i in range(5):
        cartPole = MountainCar('MountainCar-v0', os.path.join(root, f'MountainCar_2_{i + 1}'))
