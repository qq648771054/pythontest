...
class PPO(object):
    ...
    def __init__(self):
        ...
        # 构建critic网络：
        # 输入state，输出V值
        tfs = tf.keras.layers.Input([S_DIM, ], dtype=tf.float32, name='state')
        l1 = tf.keras.layers.Dense(100, tf.nn.relu)(tfs)
        v = tf.keras.layers.Dense(1)(l1)
        self.critic = tf.keras.Model(inputs=tfs, outputs=v)
        # self.critic.compile(
    ...
    def _build_anet(self, name, trainable):
        ...
        '''
        # 连续动作型问题，输出mu和sigma。
        tfs = tf.keras.layers.Input([S_DIM, ], dtype=tf.float32, name=name + '_state')
        l1 = tf.keras.layers.Dense(100, tf.nn.relu, name=name + '_l1')(tfs)

        a = tf.keras.layers.Dense(A_DIM, tf.nn.tanh, name=name + '_a')(l1)
        mu = tf.keras.layers.Lambda(lambda x: x * 2, name=name + '_lambda')(a)

        sigma = tf.keras.layers.Dense(A_DIM, tf.nn.softplus, name=name + '_sigma')(l1)

        model = tf.keras.Model(inputs=tfs, outputs=[mu, sigma], name=name)
